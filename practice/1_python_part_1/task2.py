...
def set_to_dict(dict_to_update: Dict[str, int], **items_to_set) -> Dict:
    for kwarg in items_to_set:
        if len(list(dict_to_update.keys())) == 0:
            dict_to_update = dict(**items_to_set)
        elif dict_to_update[kwarg] < items_to_set[
            kwarg]:
            dict_to_update[kwarg] = items_to_set[kwarg]
